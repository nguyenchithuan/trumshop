...
export default function FeaturedCatalogSection({ catalogHref, language }: FeaturedCatalogSectionProps) {
  const products = ["chatgpt", "gemini", "kling", "capcut"].map((id) => catalogProducts.find((product) => product.id === id)).filter(Boolean);
  ...
  return <section className="featured-catalog section shell scroll-reveal" id="san-pham-noi-bat">
    ...
    <div className="featured-catalog-grid">
      {products.map((product, index) => product && <a className={`featured-catalog-card ${product.soldOut ? "is-sold-out" : ""}`} href={`${catalogHref}?product=${product.id}`} key={product.id} style={{ "--featured-accent": product.accent, "--featured-delay": `${index * 85}ms` } as CSSProperties}>
